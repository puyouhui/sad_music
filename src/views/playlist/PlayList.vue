<template>
  <div>
    <NavBar>
      ...
      <span slot="center" class="iconfont title_center"><i>歌单</i></span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      capitalize(data) {
        for (var i = 0; i < data.length; i++) {
          if (i > (data.length - 2)) {
            this.all_id = this.all_id + data[i].id
          } else {
            this.all_id = this.all_id + data[i].id + ','
          }
        }
    ...
    created() {
      this.cover_word = JSON.parse(localStorage.getItem('StorePlayList'))
      console.log(this.cover_word);
      getSongList(this.cover_word.id)
        .then(res => {
          this.song_id = res.data.playlist.trackIds
          this.capitalize(this.song_id) //获取所有歌曲id
          getSongDetailed(this.all_id) //请求歌曲列表
            .then(res => {
              // console.log(res);
              console.log(this.all_id);
              this.song_list = res.data.songs
